...
public sealed class D20Session : IDisposable
{
    ...
    public static readonly StatId MaximumVitalityStat = StatId.Parse("d20.maximum-vitality");
    public static readonly TrackId VitalityTrack = TrackId.Parse("d20.vitality");
    ...
    public EntityId AddParticipant(CharacterDefinition character, EncounterFaction faction, int? vitality = null)
    {
        ...
        EntityBatch batch = new EntityBatch()
            .Create(entity)
            .Set(entity, D20ComponentTypes.Abilities, abilities)
            .Set(entity, D20ComponentTypes.Resources, resources)
        ...
        _participantCharacters.Add(entity, character.Id); _ownerEntities.Add(character.Id, entity); _ownerIds.Add(entity, character.Id); _ownerKinds[character.Id] = SessionOwnerKind.Participant; _ownerCapacities[character.Id] = Tuning.ParticipantInventoryCapacity; Revision++; return entity;
    }
    ...
    public IReadOnlyList<DefeatRecoveryReceipt> ApplyDefeatRecovery(IEnumerable<EntityId> partyMembers, int amount)
    {
        ...
        {
            if (!HasVitality(entity) || Entities.Get(entity, D20ComponentTypes.Participation).Faction != EncounterFaction.Party) throw new D20SessionException("Defeat recovery may target only admitted party participants.");
    ...
    public ReactionResolutionReceipt ResolveReaction(ActionPreview preview, D20Id? reaction)
    {
        ...
        {
            batch.Set(freshPreview.Actor, D20ComponentTypes.Budgets, afterActionBudgets);
        }
        else
        {
            batch.Set(freshPreview.Target, D20ComponentTypes.Budgets, afterReactionBudgets)
                .Set(freshPreview.Actor, D20ComponentTypes.Budgets, afterActionBudgets);
        }
        if (targetDies)
        {
            EncounterParticipationFact targetParticipation = Entities.Get(freshPreview.Target, D20ComponentTypes.Participation);
    ...
    public void AdvanceTurn()
    {
        ...
        foreach (EntityComponent<EffectProjectionFact> component in Entities.Query(D20ComponentTypes.Effects))
        {
            ...
            EffectProjectionFact after = new(component.Value.Values.Where(value => value.ExpiresAtTurn > nextTurn).ToImmutableArray());
