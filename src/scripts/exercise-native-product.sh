...
for source_assertion in 'const tacticalControls' "tactical.setAttribute('aria-modal', 'true')" 'background.inert = true' 'event.key === '\''Tab'\''' 'campaignButtons.forEach'; do grep -Fq "$source_assertion" "$host_root/browser-source/ui/main.js"; done
start_host first
