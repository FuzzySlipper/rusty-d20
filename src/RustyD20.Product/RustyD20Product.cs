...
public sealed class RustyD20Product : IEngineProduct
{
    ...
    private void AddCampaign(List<UiField> fields, CampaignSnapshot state)
    {
        ...
        if (_session is { } session) { fields.Add(UiField.TextValue("party.vitality", string.Join(';', _content.Adventures[_selection].Party.Select(id => $"{id}:{session.ReadVitality(session.OwnerEntity(id)).Current}")))); fields.Add(UiField.TextValue("party.loadout", string.Join(',', _content.Adventures[_selection].Items.Take(12).Select(value => value.Value)))); fields.Add(UiField.TextValue("rolls", string.Join(';', session.Receipts.TakeLast(4).Select(value => $"{value.D20}/{value.Damage}")))); }
    }
...
internal sealed class D20Surface : IDisposable
{
    ...
    private static Material CreateMaterial(IEngineContext engine, Color color, float roughness) => engine.Graphics.CreateMaterial(new MaterialRequest(color, default(RenderResourceReference), roughness, new Color(1, 1, 1, 1), Vector3.Zero, 0, false));
