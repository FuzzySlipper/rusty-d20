const controls = [
  ...
  ['Move north', 'd20.tactical.move.north'], ['Move south', 'd20.tactical.move.south'], ['Move west', 'd20.tactical.move.west'], ['Move east', 'd20.tactical.move.east'], ['Party', 'd20.party.next'], ['Action', 'd20.action.next'], ['Target', 'd20.target.next'], ['Commit action', 'd20.action.commit'],
...
export function mountProductUi(root, context) {
  ...
  const observerList = document.createElement('dl'); observerList.style.cssText = 'display:grid;grid-template-columns:max-content 1fr;gap:.35rem 1rem'; observations.append(observerList); panel.append(observations);
  const tactical = document.createElement('section'); tactical.setAttribute('role', 'dialog'); tactical.setAttribute('aria-modal', 'true'); tactical.setAttribute('aria-label', 'Tactical workbench observer'); tactical.hidden = true; tactical.style.cssText = 'border:2px solid #8a6510;padding:1rem;margin-top:1rem;background:#17130b';
  ...
  const logSection = document.createElement('section'); const logTitle = document.createElement('h2'); logTitle.textContent = 'Resolution log'; logSection.append(logTitle); const log = document.createElement('ol'); log.setAttribute('aria-live', 'polite'); logSection.append(log); panel.append(logSection); root.append(panel);
  ...
  const render = (envelope) => {
    ...
    const tacticalFields = entries.filter(([key]) => key.startsWith('tactical.') || key.startsWith('selection.'));
    tactical.hidden = phase !== 'Encounter'; if (!tactical.hidden) renderFields(tacticalList, tacticalFields);
  ...
  const onKeydown = (event) => { const intent = ({ w: 'd20.forward', s: 'd20.back', a: 'd20.left', d: 'd20.right', e: 'd20.interact', ArrowUp: 'd20.tactical.move.north', ArrowDown: 'd20.tactical.move.south', ArrowLeft: 'd20.tactical.move.west', ArrowRight: 'd20.tactical.move.east', Escape: 'd20.reaction.decline' })[event.key]; if (intent) { event.preventDefault(); emit(intent); } };
