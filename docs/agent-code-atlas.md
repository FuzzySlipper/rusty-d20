...
| `rust/crates/rusty-d20/src/game/persistence.rs` | Strict schema-7 save/reopen with exploration state, exact composition/history binding, and schemas 1–6 migration | `cargo test -p rusty-d20 --lib --locked` |
...
| `libs/components`, `libs/renderer`, `libs/ui-compass`, `libs/ui-minimap` | Product-neutral presentation building blocks; renderer owns the first-person corridor and compass/minimap consume only Rust-projected navigation facts | Typecheck, lint, Playwright |
